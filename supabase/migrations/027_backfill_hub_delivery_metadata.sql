-- Catch-up migration for databases that missed the 021 delivery metadata rollout.
...
begin
	if not exists (
		select 1
		from pg_constraint
		where conname = 'hub_broadcasts_delivery_state_check'
			and conrelid = 'public.hub_broadcasts'::regclass
	) then
		alter table public.hub_broadcasts
...
begin
	if not exists (
		select 1
		from pg_constraint
		where conname = 'hub_events_delivery_state_check'
			and conrelid = 'public.hub_events'::regclass
	) then
		alter table public.hub_events
