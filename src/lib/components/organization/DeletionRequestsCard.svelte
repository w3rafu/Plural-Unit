...
{#if currentOrganization.isLoadingDeletionRequests && currentOrganization.deletionRequests.length === 0}
	<div class="rounded-2xl border border-border/70 bg-background/78 px-3 py-2">
		<p class="text-[0.72rem] text-muted-foreground">Loading pending requests…</p>
	</div>
{:else if currentOrganization.deletionRequests.length === 0}
	<!-- no pending deletion requests -->
{:else}
	{@const request = currentOrganization.deletionRequests[0]}
	<section class="rounded-2xl border border-border/70 bg-background/78 px-2.75 py-2 shadow-sm">
		<div class="flex flex-col gap-1.5 sm:flex-row sm:items-center sm:justify-between">
			<div class="min-w-0 space-y-0.5">
				...
				<p class="text-[0.72rem] leading-4.5 text-muted-foreground">
			...
			<Button
				...
				variant="ghost"
				size="sm"
				class="h-7 rounded-full px-2.75 text-[0.7rem] text-foreground hover:bg-muted/70"
