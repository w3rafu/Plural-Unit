<script lang="ts">
	...
	const resolvedEvents = $derived(events ?? currentHub.liveEvents);
	...
	<Card.Header class="gap-3 border-b border-border/70">
		<div class="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between">
			<div class="space-y-1">
				...
				<Card.Description>
					See what still needs a reply and what is already on your calendar.
				</Card.Description>
			</div>
			...
			<div class="flex flex-wrap gap-2">
				...
				<Badge variant="secondary">{commitments.upcomingCount} upcoming</Badge>
			</div>
		</div>
	</Card.Header>
	...
	<Card.Content class="space-y-4">
		{#if commitments.all.length === 0}
			<div class="rounded-2xl border border-dashed border-border/70 bg-muted/20 px-4 py-5">
				...
				<p class="mt-1 text-sm text-muted-foreground">
					When new events go live, this card will track what still needs a reply and what is
		...
		{:else}
				...
				<div class="space-y-3">
					<div class="space-y-1">
						...
						<p class="text-sm text-muted-foreground">
							Lock in the events that still need your answer.
						</p>
					</div>
			...
			{#if upcomingPreview.length > 0}
				<div class="space-y-3">
					<div class="space-y-1">
						<h3 class="text-sm font-semibold tracking-tight text-foreground">Upcoming commitments</h3>
						<p class="text-sm text-muted-foreground">
							Keep an eye on the events you already plan to attend.
						</p>
					</div>
