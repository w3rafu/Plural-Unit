<script lang="ts">
	...
	<section class="grid gap-2.5 lg:grid-cols-[minmax(0,1.88fr)_13.5rem] xl:grid-cols-[minmax(0,1.98fr)_13.75rem]">
	<Card.Root class="relative overflow-hidden border-border/70 bg-linear-to-br from-card via-background to-primary/5.5 shadow-sm">
		...
		<Card.Content class="relative space-y-2.25 py-3 sm:py-3.5">
			...
			<div class="grid gap-1.75 border-t border-border/60 pt-2 2xl:grid-cols-[minmax(0,1fr)_13.5rem] 2xl:items-start">
				<div class="space-y-1">
				...
				{#if spotlightPeople.length > 0}
					<div class="space-y-1.5 border-t border-border/60 pt-2 2xl:border-t-0 2xl:pt-0">
		...
		<Card.Content class="space-y-2 py-3">
			<div class="flex items-center justify-between gap-3">
				<div>
					...
					<p class="mt-0.75 text-[0.9rem] font-semibold tracking-tight text-foreground">How things are moving</p>
				</div>
			...
			<div class="space-y-1.5">
				{#each signalRows as row (row.label)}
					<div class="space-y-0.75">
						<div class="flex items-center justify-between gap-3">
							<div>
								<p class="text-[0.9rem] font-medium text-foreground">{row.label}</p>
								<p class="text-[0.7rem] leading-4.5 text-muted-foreground">{row.note}</p>
							</div>
							<span class="text-[0.9rem] font-semibold text-foreground">{row.value}%</span>
						</div>
			...
			<div class="rounded-[1rem] bg-muted/20 px-2.75 py-2.25">
				<p class="text-[0.74rem] leading-4.5 text-muted-foreground">{activityCaption}</p>
				<p class="mt-1 text-[0.88rem] font-medium text-foreground">
					{unreadMessages > 0
						? `${formatCount(unreadMessages, 'conversation')} still need a reply.`
						: 'Messages are caught up for now.'}
				</p>
				<p class="mt-0.75 text-[0.74rem] leading-4.5 text-muted-foreground">
					{liveBroadcasts > 0 || upcomingEvents > 0
