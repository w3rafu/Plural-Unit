<script lang="ts">
	...
	<section class="grid gap-4 lg:grid-cols-[minmax(0,1.78fr)_15.25rem] xl:grid-cols-[minmax(0,1.88fr)_15.75rem]">
	<Card.Root class="relative overflow-hidden border-border/70 bg-linear-to-br from-card via-background to-primary/5.5 shadow-sm">
		...
		<Card.Content class="relative space-y-3.25 py-4 sm:py-4.5">
			<div class="space-y-1">
				<p class="text-[0.68rem] font-semibold uppercase tracking-[0.18em] text-muted-foreground">Hub focus</p>
				<h2 class="max-w-2xl text-[1.72rem] font-semibold tracking-tight text-foreground sm:text-[2.02rem] sm:leading-[1.02]">
					{attentionLabel}
				</h2>
			...
			<div class="flex flex-col gap-2">
				{#each decisionRows as row (row.label)}
					<div class="rounded-[1.1rem] border border-border/70 bg-background/88 px-3.5 py-2.5 shadow-sm">
						<div class="flex items-start justify-between gap-3">
							<p class="text-sm font-semibold text-foreground">{row.label}</p>
							<p class="shrink-0 text-[1.5rem] leading-none font-semibold tracking-tight text-foreground">{row.value}</p>
						</div>
						<p class="mt-1 text-[0.82rem] leading-5 text-muted-foreground">{row.summary}</p>
					</div>
				{/each}
			</div>

			<div class="grid gap-2.5 border-t border-border/60 pt-3 2xl:grid-cols-[minmax(0,1fr)_15.5rem] 2xl:items-start">
				<div class="space-y-1">
				...
				{#if spotlightPeople.length > 0}
					<div class="space-y-2 border-t border-border/60 pt-2.5 2xl:border-t-0 2xl:pt-0">
							...
							{#each spotlightPeople as person (person.id)}
								<div class="flex items-center gap-3">
									...
									<div class="min-w-0 flex-1">
										<p class="truncate text-[0.84rem] font-medium text-foreground">{person.name}</p>
										<p class="truncate text-[0.72rem] text-muted-foreground">{person.subtitle}</p>
									</div>
