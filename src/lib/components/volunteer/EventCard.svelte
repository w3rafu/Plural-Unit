	...
	<Card.Content class="space-y-3.5 p-3.5">
		<div class="grid grid-cols-[auto,minmax(0,1fr)] gap-3">
			<div class="flex h-12 w-12 shrink-0 flex-col items-center justify-center rounded-2xl border border-border/70 bg-background/85 text-center shadow-sm">
				<p class="text-[0.86rem] font-semibold uppercase tracking-[0.18em] text-muted-foreground">{getDateParts(event.date).month}</p>
			...
			<div class="min-w-0 space-y-2">
				<div class="flex flex-wrap items-start justify-between gap-2">
					<div class="min-w-0">
						<p class="text-[0.88rem] font-semibold tracking-tight text-foreground">{event.title}</p>
						<p class="mt-0.5 text-[0.8rem] text-muted-foreground">{event.date} · {event.timeRange}</p>
					</div>
					<FillPill {filled} needed={total} {status} />
				</div>

				<div class="flex flex-wrap items-center gap-x-3 gap-y-1 text-[0.88rem] text-muted-foreground">
			...
			<div class="flex items-center justify-between gap-3 rounded-2xl bg-muted/18 px-3 py-2">
				...
				{#if visibleParticipants.length > 0}
					<div class="flex items-center gap-2">
						<div class="flex -space-x-2">
							{#each visibleParticipants as participant (participant.id)}
								<Avatar.Root class="size-7 border-2 border-card bg-background shadow-sm after:hidden">
									...
									{:else}
										<Avatar.Fallback class="text-[0.75rem] font-semibold text-foreground">
						...
						<p class="text-[0.8rem] text-muted-foreground">{visibleParticipants.length} returning</p>
					</div>
				{/if}
			</div>
		{/if}
		...
		<div class="flex items-center justify-between gap-3 border-t border-border/60 pt-3">
			<p class="text-[0.86rem] text-muted-foreground">Board is open now.</p>
			<Button href={signupHref} variant="default" size="sm" class="h-8 rounded-full px-4 text-[0.82rem] shadow-sm">Join shift</Button>
		</div>
	</Card.Content>
</Card.Root>
